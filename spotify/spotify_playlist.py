...
['the+truth+will+set+you+free',
 ...
 'you+free']
    ...
    phrases = [phrase]
    for k in range(int(np.floor(n_words/2)+1),0,-1): #constructs a list of possible phrases you can make
        #print k
...
example: song_name, song_length, artist, popularity, url = get_json("I'm+gonna+get+free")
    ...
    for j in range(n_songs): #sets the values for song_names, length etc
        ...
        song_length[j] = len(song_names[j])
...
def spotify_poetry(phrase, alpha=0.1, beta=1, tot_songs=50, wt=2.):
    """
...
example: sn, sl, su, sp, an, chis, p = spotify_poetry("the truth will set you free",alpha=0.1,beta=1,tot_songs=50, wt=1.)
    """
    ...
    if not os.path.exists('/home/tim/spotify'):
        ...
        os.mkdir('/home/tim/spotify/examples')
    elif not os.path.exists('/home/tim/spotify/examples'):
        os.mkdir('/home/tim/spotify/examples')
    outfile = '/home/tim/spotify/examples/'+phrase.replace(' ','_')+'.txt' #creates the outfile for the spotify urls
    outfile_ivy = '/home/tim/spotify/examples/'+phrase.replace(' ','_')+'_ivy.txt' #creates the outfile for the spotify urls
    ...
    n_songs = np.size(song_name)
    u_songs = np.unique(song_artist) #gets only unique song names
    ...
    for j in range(n_songs): #find unique songs in song list
        for k in range(n_u_songs):
            if song_artist[j] == u_songs[k]: #find the popularity of the song if theres a match
    ...
    if not alpha or not beta: #if the coefficients don't exist, set them
        ...
        for j in range(int(tot_songs/5.),n_u_songs): #non zero starting point to avoid the trivial solution at n_tracks = 0
...
#    return u_songs, u_length, u_url, u_popularity, u_aname, phrase
